...
*/
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        if indexPath.row == 3 {
            performSegueWithIdentifier("suspendedStatusSegue", sender: self)
        }else if indexPath.row == 4 {
            performSegueWithIdentifier("rescheduledStatusSegue", sender: self)
        }else if indexPath.row == 5 {
            performSegueWithIdentifier("noActionStatusSegue", sender: self)
        }else{
            dismissViewControllerAnimated(true, completion: nil)
            self.delegate?.backFromSelectedStatus(indexPath.row)
        }
